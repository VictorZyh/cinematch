...
- Package-first Python structure.
- No notebooks in the production path.
- Only `pandas`, `numpy`, and `scikit-learn` for data and ML logic.
- Modular components with clear ownership.
- Explicit type hints and docstrings.
- Concise code comments around key ML logic.
- Time-based splitting and train-only feature computation to avoid leakage.
- Unit-tested functions and reproducible command-line scripts.

## Next Improvements

- Add feature importance and diagnostics.
- Add stronger hard-negative sampling.
- Add experiment tracking with metrics history.
- Add API serving once the offline and batch pipelines are stable.
