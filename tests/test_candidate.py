...
def test_bpr_generator_scores_unseen_items() -> None:
    interactions = _interactions()
    generator = BPRCandidateGenerator(
        num_factors=3,
        num_epochs=2,
...
def test_bpr_generator_requires_fit() -> None:
    with pytest.raises(RuntimeError, match="must be fit"):
        BPRCandidateGenerator().generate([1], {}, 10)
